    ...
    <section id="live-preview" className="relative py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="text-center max-w-3xl mx-auto mb-12">
        ...
        <h2 className="text-3xl sm:text-5xl font-extrabold text-foreground tracking-tight">
          Experience the <span className="text-primary">AetherSync Workspace</span> Live
        </h2>
      ...
      <div className="relative rounded-2xl border border-border bg-background shadow-2xl shadow-primary/20 overflow-hidden ">
        ...
        <div className="flex h-10 w-full items-center justify-between border-b border-border bg-background px-3.5 text-xs select-none">
          ...
          <div className="flex items-center gap-2.5">
            <div className="w-5 h-5 rounded-md bg-primary flex items-center justify-center font-bold text-foreground text-[11px] shadow-sm">
              A
            </div>
            ...
            <div className="h-3.5 w-[1px] bg-surface-elevated mx-1 hidden sm:block" />
            ...
            <span className="text-text-secondary font-medium text-[11px] hidden md:inline">frontend</span>
          </div>

          {/* Center: Command Palette Pill */}
          <div className="hidden lg:flex items-center justify-between w-72 rounded-full border border-border bg-surface-elevated px-3 py-1 text-[11px] text-text-secondary shadow-inner">
            <div className="flex items-center gap-1.5 truncate">
              <Search size={11} className="text-muted" />
              <span className="truncate">Search files, symbols & commands...</span>
            </div>
            <kbd className="rounded bg-black/40 px-1 py-0.2 font-mono text-[9px] text-muted border border-border">
              Ctrl+P
            </kbd>
          </div>
          ...
          <div className="flex items-center gap-2">
            <select
              ...
              className="bg-surface-elevated border border-border text-primary text-[11px] font-medium rounded-lg px-2 py-0.5 outline-none cursor-pointer"
            >
        ...
        <div className="flex h-[520px] w-full overflow-hidden text-xs">
          ...
          <div className="w-48 sm:w-56 shrink-0 border-r border-border bg-surface p-2 flex flex-col justify-between select-none">
            <div>
              <div className="flex items-center justify-between px-2 py-1 text-[10px] font-bold text-muted uppercase tracking-wider">
                <span>Explorer: FRONTEND</span>
              </div>

              {/* Tree */}
              <div className="mt-1 space-y-0.5">
                <div className="flex items-center gap-1 px-2 py-1 text-text-secondary font-semibold text-[11.5px]">
                ...
                <div className="pl-4 space-y-0.5">
                  <div className="flex items-center gap-1.5 px-2 py-1 text-text-secondary text-[11px]">
                    <ChevronRight size={11} className="text-slate-600" />
                    <Folder size={12} className="text-muted" />
                    <span>public</span>
                  </div>

                  <div className="flex items-center gap-1 px-2 py-1 text-text-secondary font-medium text-[11.5px]">
                    <ChevronDown size={12} className="text-muted" />
            ...
            {/* Bottom Status */}
            <div className="px-2 py-2 border-t border-border text-[10px] text-muted flex items-center justify-between">
          ...
          <div className="flex flex-1 flex-col min-w-0 bg-surface">
            ...
            <div className="flex-1 overflow-y-auto p-4 font-mono text-[11.5px] leading-relaxed text-text-secondary selection:bg-primary/30">
              {filesData[activeFile].lines.map((line, idx) => (
                <div key={idx} className="flex gap-4 hover:bg-border px-1 rounded">
            ...
            <div className="h-40 border-t border-border bg-background flex flex-col">
              <div className="flex h-7 items-center justify-between border-b border-border bg-surface px-3 text-[10px] text-text-secondary font-mono">
                <div className="flex items-center gap-2">
              ...
              {/* Terminal Output */}
              <div className="flex-1 overflow-y-auto p-2.5 font-mono text-[11px] text-text-secondary space-y-1">
                ...
                <form onSubmit={handleTerminalSubmit} className="flex items-center gap-1.5 pt-1">
                  ...
                  <input
                    ...
                    className="flex-1 bg-transparent text-foreground outline-none font-mono text-[11px]"
                  />
          ...
          {copilotOpen && (
            <div className="w-64 sm:w-72 shrink-0 border-l border-border bg-surface-elevated flex flex-col justify-between">
              ...
              <div className="flex h-9 items-center justify-between border-b border-border bg-surface px-3">
                ...
                <span className="rounded bg-primary/20 text-primary text-[9px] px-1.5 py-0.5 font-mono">
                  ACTIVE
                </span>
              </div>
                ...
                {copilotMessages.map((msg, i) => (
                  <div
                    ...
                    className={`rounded-xl p-2.5 text-[11px] leading-relaxed ${
                      msg.role === 'user'
                        ? 'bg-primary/15 border border-primary/30 text-primary/60 ml-3'
                        : 'bg-surface-elevated border border-border text-text-secondary mr-1'
                    }`}
                  >
                    <div className="font-semibold text-[10px] text-muted mb-1 uppercase tracking-wider">
              ...
              <form onSubmit={handleCopilotSubmit} className="p-2.5 border-t border-border bg-surface">
                <div className="flex items-center gap-1.5 rounded-xl bg-surface-elevated border border-border px-2.5 py-1.5">
                  <input
                    type="text"
                    value={copilotInput}
                    onChange={(e) => setCopilotInput(e.target.value)}
                    placeholder="Ask about this file or agent task..."
                    className="flex-1 bg-transparent text-foreground outline-none text-[11px]"
                  />
